...
- P2-004 已实现两端本地直接移除、全构建允许截屏、“先配对、后共享、Windows 手动连接”，并收口配对后导航、停止共享后的配对记录、手机存储根目录、返回/退出语义和 Windows 本地名称/备注。自动验证与 Samsung 定向实测通过，等待用户真实跨端验收。多设备核心尚未开始。
...
- Windows r14 验收窗口已打开，使用独立 `0.2.0.14` 数据根且配对数为 0；Android `org.phonebridge.ng.uipreviewr14` 已清除数据并停在无身份、无配对、共享停止的首页。制品和散列见 `docs/UI_ACCEPTANCE.md`。
...
## 8. 唯一下一任务：使用 r14 完成 P2-004 真实跨端验收

### 目标

从 r14 的全新状态完成“Android 配对新电脑 → Windows 添加手机并输入配对码 → Android 批准 → Android 开始共享 → Windows 手动连接”，并验证停止共享后的电脑记录、手机存储浏览、Windows 本地名称/备注、Android 返回/后台/显式退出，最后分别从 Windows 与 Android 各执行一次本地移除并全新重配。通过前不开始 P2-005 多设备核心。

### 必须交付

- 配对完成后确认 Windows 返回设备首页且没有自动挂载；Android 停止共享时仍显示已配对电脑；手机开始共享后必须由用户在 Windows 手动点击“连接”。
- 验证原有共享目录仍在、“手机存储”可浏览系统允许的内部共享存储顶层；共享中退到后台继续共享，显式退出应用停止共享。
- Windows 保存本地名称和备注后，返回首页和重启 r14 均保持，且设备身份不变。
...
3. P2-004：两端本地直接移除、允许截屏和配对体验收口（已实现，等待 r14 真实跨端验收）。
...
v0.1.0 已发布。P2-002 静态设计和 P2-003 实际 UI 已由用户确认。P2-004 已实现两端本地直接移除、全构建允许截屏和“先配对、后共享、Windows 手动连接”，并加入配对成功返回首页、停止共享后保留电脑记录、手机存储、Android 返回/后台/显式退出以及 Windows 本地名称/备注。每轮验收候选使用新的 Windows 数据根与 Android 应用 ID。当前 r14 已以全新状态部署，等待真实跨端验收。多设备核心顺延为 P2-005。Android 永久只通过 GitHub 分发。保持现有 Kotlin/WPF、HTTPS/WebDAV、rclone、WinFsp、mDNS、配对与安全存储路线。

本轮只完成 P2-004 r14 真实跨端验收：按 `docs/UI_ACCEPTANCE.md` 验证配对后导航、后共享及 Windows 手动连接、停止共享后的电脑记录、手机存储浏览、Windows 本地名称/备注、Android 生命周期，最后分别验证 Windows 和 Android 本地移除、旧凭据失效和全新重配。不要开始 P2-005 多设备核心，不把候选当正式发布包，不扩展功能或测试。完成后把状态、未验证项和唯一下一任务写入仓库并提交推送。
```
