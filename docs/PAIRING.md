...
前提：用户从自己手机的当前配对界面取得秘密短码。看到/控制该屏幕的攻击者、已控制 Windows 用户会话或 Android 应用进程的攻击者不在 PAKE/本地保险库可独立防御的范围。手机确认不能补救已泄露短码被用于冒充另一台服务端的情况；不得宣传为防已控制终端。产品配对页需避免截图/日志/遥测暴露码，普通连接不再要求输入密码。
...
状态分开：Windows `Pending → Active → RevocationPending/NeedsRepair`；Android `PendingApproval → Active → Revoked`，Rejected/Cancelled/Expired 不授权文件。短码正确或 CA 正确不等于用户已批准，配对 Active 也不等于存储权限/共享服务就绪。
...
- Windows 本地撤销：先持久禁用该记录的连接和活动恢复，停止本会话，再请求 self 撤销并回读。离线时保留受保护待撤销记录供明确重试；“仅从本机忘记”必须告知手机可能仍有授权，不能显示成远端撤销成功。
