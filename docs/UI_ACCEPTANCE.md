...
状态：P2-006 r21 UI、单卡状态与语言收口已通过限定验收并由 P2-007 转入正式 `0.2.0`。本文件记录 Git 忽略的历史 `.audit/ui-acceptance/` 候选，它们不是当前安装入口。
...
- r21 只用于历史隔离验收；正式签名 APK、Windows 安装器和 `0.2.0` 首个升级基线已由 P2-007 完成。
- r21 Android 真实前台通知外观未触发；实现、双语资源、构建和 lint 已通过。
- P2-007 全新安装后，正式 Windows 与 Samsung 已重新建立一条配对记录。
