...
Android 使用 `adb install -r` 覆盖安装时，只有签名证书相同才保留配对与设置。测试签名 APK不能直接覆盖正式签名 APK；首次切换签名必须先卸载旧包，因此配对记录不能跨签名迁移。`0.2.0` 是首个正式升级基线；以后每个新正式候选必须动态使用候选之前最近一个正式 Release 作为升级来源，并继续使用同一长期私钥和证书，不得固定某个历史版本。
