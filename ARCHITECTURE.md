...
状态优先级固定为错误、已挂载、连接中、已发现、离线；文本来自 zh-CN/en-US 资源。当前桌面进程只有一个 `ConnectionClient`，它只拥有一个 `MountManager`，因此多台手机可同时被发现并保留配对记录，但一次只能连接和映射一台。增加同时多设备挂载需要独立的每设备会话、盘符、恢复和托盘模型，当前版本未实现。
