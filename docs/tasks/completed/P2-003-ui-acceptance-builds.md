...
- P2-003 验收时仅 `uiPreview` 允许截图；正式构建当时仍设置 `FLAG_SECURE`。该策略后来由 P2-004 按用户新要求整体取消。
...
P2-004 按用户新要求实现两端本地直接移除配对信息并取消 Android 截屏阻挡；多设备核心顺延。
