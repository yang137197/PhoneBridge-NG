...
- 手机批准后，Active 配对结果必须保持到原 120 秒窗口期限，使 Windows 能完成状态轮询和严格 session 验证；Android 返回首页、进入权限页或暂时后台不得提前销毁已批准结果。
...
- r14 真实流程确认 Android UI 在观察到 Active 后立即调用 `closePairing`，使 Windows 下一次轮询得到 Unauthorized，留下 Pending 并显示“待确认/继续连接”。r15 不再关闭 Active 窗口；前后台切换和普通关闭只取消尚未批准的窗口，Active 结果按既有协议保留到原期限，配对专用服务在窗口自然结束后自动停止。
