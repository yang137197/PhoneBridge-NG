...
- P2-004 已实现两端本地直接移除、全构建允许截屏、“先配对、后共享、Windows 手动连接”，并收口配对后导航、停止共享后的配对记录、手机存储根目录、返回/退出语义和 Windows 本地名称/备注。r14 真实流程发现手机批准后过早关闭 Active 结果的竞态；r15 已修复并通过自动验证与 Samsung 定向测试，等待用户复验。多设备核心尚未开始。
...
- Windows r15 使用独立 `0.2.0.15` 数据根；Android `org.phonebridge.ng.uipreviewr15` 已按新包名全新安装，不继承 r14 身份或配对记录。制品和散列见 `docs/UI_ACCEPTANCE.md`。
...
| V2-METADATA-01 | Windows 保存设备本地名称和备注 | 重启后保持，不改变设备身份和手机端名称 |
...
- Android 共享引擎停止后仍独立读取加密配对库，所以已配对电脑可继续显示、修改访问模式和本地删除；共享目录保留既有七项并追加内部共享存储根目录。
- Windows 配对记录 schema 2 增加本地名称和备注并向后读取 schema 1；配对成功后返回设备首页。本地字段不参与身份或凭据校验。
...
## 8. 唯一下一任务：使用 r15 复验 P2-004 真实跨端流程

### 目标

从 r15 的全新状态首先确认：Android 批准后 Windows 自动收到 Active、验证 session 并返回设备首页，直接显示“已配对、未连接/连接”，不得出现“待确认/继续连接”。随后完成 Android 开始共享、Windows 手动连接，并验证停止共享后的电脑记录、手机存储浏览、Windows 本地名称/备注、Android 返回/后台/显式退出，最后分别从 Windows 与 Android 各执行一次本地移除并全新重配。通过前不开始 P2-005 多设备核心。

### 必须交付

- 配对完成后确认 Windows 自动返回设备首页、不显示 Pending/继续连接且没有自动挂载；Android 停止共享时仍显示已配对电脑；手机开始共享后必须由用户在 Windows 手动点击“连接”。
- 验证原有共享目录仍在、“手机存储”可浏览系统允许的内部共享存储顶层；共享中退到后台继续共享，显式退出应用停止共享。
- Windows 保存本地名称和备注后，返回首页和重启 r15 均保持，且设备身份不变。
...
配对后导航、先配对后共享、手动连接、停止共享后的电脑记录、手机存储浏览、本地名称/备注和 Android 生命周期均通过；两端真实移除不依赖远端确认，旧凭据失效，全新重配成功。P2-004 完成前不启动多设备重构。

## 9. 后续顺序

1. P2-002：已完成并经用户确认。
2. P2-003：两端原生 UI 验收（已完成并由用户复验通过）。
3. P2-004：两端本地直接移除、允许截屏和配对体验收口（r15 已修复批准结果竞态，等待真实复验）。
...
v0.1.0 已发布。P2-002 静态设计和 P2-003 实际 UI 已由用户确认。P2-004 已实现两端本地直接移除、全构建允许截屏和“先配对、后共享、Windows 手动连接”，并加入配对成功返回首页、停止共享后保留电脑记录、手机存储、Android 返回/后台/显式退出以及 Windows 本地名称/备注。r14 发现 Android 过早关闭已批准结果，Windows 留在 Pending；r15 按既有协议保留 Active 到原窗口期限并已通过自动验证与 Samsung 定向测试。每轮验收候选使用新的 Windows 数据根与 Android 应用 ID。当前 r15 已全新安装，等待真实复验。多设备核心顺延为 P2-005。Android 永久只通过 GitHub 分发。保持现有 Kotlin/WPF、HTTPS/WebDAV、rclone、WinFsp、mDNS、配对与安全存储路线。

本轮只完成 P2-004 r15 真实跨端复验：按 `docs/UI_ACCEPTANCE.md` 先确认手机批准后 Windows 自动回首页且不再出现“待确认/继续连接”，再验证后共享及 Windows 手动连接、停止共享后的电脑记录、手机存储浏览、Windows 本地名称/备注、Android 生命周期，最后分别验证 Windows 和 Android 本地移除、旧凭据失效和全新重配。不要开始 P2-005 多设备核心，不把候选当正式发布包，不扩展功能或测试。完成后把状态、未验证项和唯一下一任务写入仓库并提交推送。
```
