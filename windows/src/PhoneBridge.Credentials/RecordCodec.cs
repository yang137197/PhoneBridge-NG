...
internal static class RecordCodec
{
    ...
    internal static byte[] Encode(StoredRecord record)
    {
        ...
        var result=new byte[4+1+8+1+1+2+ca.Length+32+69+16+32+2+device.Length+2+client.Length+2+alias.Length+2+note.Length];
        int p=0;
        ...
        Put("PBC1"u8.ToArray());result[p++]=2;BinaryPrimitives.WriteUInt64BigEndian(result.AsSpan(p),m.Revision);p+=8;
        ...
        U16(device.Length);Put(device);U16(client.Length);Put(client);U16(alias.Length);Put(alias);U16(note.Length);Put(note);return result;
    }
    ...
    internal static StoredRecord Decode(byte[] plaintext,string expectedDeviceId)
    {
        try
        {
            ...
            byte version=reader.Take(1)[0];if(version is not (1 or 2))throw new InvalidDataException();
            ...
            return new StoredRecord(new PairingRecord(identity,client,deviceName,clientName,state,mode,revision,alias,note),token.ToArray());
        }
