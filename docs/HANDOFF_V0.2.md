...
- P2-004 已实现两端本地直接移除、全构建允许截屏、“先配对、后共享、Windows 手动连接”，并收口配对后导航、停止共享后的配对记录、手机存储根目录、返回/退出语义和 Windows 本地名称/备注。r14 真实流程发现手机批准后过早关闭 Active 结果的竞态；r15 已修复，配对主流程已由用户真实验收通过并确认可正常使用。其余记录管理、手机存储浏览及两端移除/重配仍待验收；多设备核心尚未开始。
...
## 8. 唯一下一任务：完成 r15 的 P2-004 剩余真实跨端验收

### 目标

r15 的配对主流程已经用户真实验收通过。继续使用现有配对记录验证停止共享后的电脑记录和访问模式、手机存储浏览、Windows 本地名称/备注及 Android 生命周期，最后分别从 Windows 与 Android 各执行一次本地移除、确认旧凭据失效并全新重配。通过前不开始 P2-005 多设备核心。

### 必须交付

- 已确认基线：配对完成后 Windows 自动收口，r14 的 Pending/“继续连接”竞态不再出现。剩余验收不得回退该行为。
...
3. P2-004：两端本地直接移除、允许截屏和配对体验收口（r15 配对主流程已真实验收通过，等待其余记录管理和移除/重配验收）。
...
v0.1.0 已发布。P2-002 静态设计和 P2-003 实际 UI 已由用户确认。P2-004 已实现两端本地直接移除、全构建允许截屏和“先配对、后共享、Windows 手动连接”，并加入配对成功返回首页、停止共享后保留电脑记录、手机存储、Android 返回/后台/显式退出以及 Windows 本地名称/备注。r14 发现 Android 过早关闭已批准结果，Windows 留在 Pending；r15 按既有协议保留 Active 到原窗口期限，并已通过自动验证、Samsung 定向测试及用户真实配对流程验收，当前可正常使用。每轮验收候选使用新的 Windows 数据根与 Android 应用 ID。记录管理、手机存储跨端浏览及两端移除/重配仍待验收。多设备核心顺延为 P2-005。Android 永久只通过 GitHub 分发。保持现有 Kotlin/WPF、HTTPS/WebDAV、rclone、WinFsp、mDNS、配对与安全存储路线。

本轮只完成 P2-004 r15 剩余真实跨端验收：按 `docs/UI_ACCEPTANCE.md` 使用现有配对记录验证停止共享后的电脑记录与访问模式、手机存储浏览、Windows 本地名称/备注、Android 生命周期，最后分别验证 Windows 和 Android 本地移除、旧凭据失效和全新重配。不要重复已经通过的配对竞态测试，不要开始 P2-005 多设备核心，不把候选当正式发布包，不扩展功能或测试。完成后把状态、未验证项和唯一下一任务写入仓库并提交推送。
```
