...
try {
    ...
    & $GradlePath -p (Join-Path $projectRoot 'android') :app:assembleDebug :app:assembleRelease :app:assembleDebugAndroidTest :app:testDebugUnitTest :app:lintDebug :app:lintRelease --dependency-verification strict --console plain
    if ($LASTEXITCODE -ne 0) { throw 'Android app build, tests or lint failed.' }
}
finally {
