...
- Windows r12 验收窗口已打开，使用独立 `0.2.0.12` 数据根且未读取正式 3 条配对记录；Android `org.phonebridge.ng.uipreviewr12` 已全新安装并停在无身份/配对的首页，旧候选已卸载。制品和散列见 `docs/UI_ACCEPTANCE.md`。
...
## 8. 唯一下一任务：使用 r12 完成 P2-004 真实端到端验收

### 目标

从 r12 的全新状态完成“Android 配对新电脑 → Windows 配对 → Android 开始共享 → Windows 手动连接”，再分别从 Windows 与 Android 各执行一次本地移除并全新重配。通过前不开始 P2-005 多设备核心。
...
3. P2-004：两端本地直接移除、允许截屏和全新配对流程（已实现，等待 r12 真实端到端验收）。
...
v0.1.0 已发布。P2-002 静态设计和 P2-003 实际 UI 已由用户确认。P2-004 已实现两端本地直接移除、全构建允许截屏和“先配对、后共享、Windows 手动连接”，并修复 Windows 配对候选迟到时未自动选中导致按钮禁用的问题；每轮验收候选使用新的 Windows 数据根与 Android 应用 ID。当前 r12 已全新部署，等待真实端到端验收。多设备核心顺延为 P2-005。保持现有 Kotlin/WPF、HTTPS/WebDAV、rclone、WinFsp、mDNS、配对与安全存储路线。

本轮只完成 P2-004 r12 真实端到端验收：按 `docs/UI_ACCEPTANCE.md` 从全新状态验证配对候选自动选中、8 位码后按钮启用、先配对后共享及 Windows 手动连接，再分别验证 Windows 和 Android 本地移除、旧凭据失效和全新重配。不要开始 P2-005 多设备核心，不把候选当正式发布包，不扩展功能或测试。完成后把状态、未验证项和唯一下一任务写入仓库并提交推送。
```
