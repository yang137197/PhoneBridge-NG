...
- r11 实机从“添加手机”先打开、无候选选中开始；手机配对广播随后到达后自动显示 `SM-S9180`，输入 8 位码后按钮真实启用并完成配对。配对完成后 rclone 为 0、P: 不存在。最终交付 r12 再次使用全新身份，当前两端配对记录均为 0。
...
用户使用 r12 全新隔离候选完成“配对新电脑 → Windows 配对 → 手机开始共享 → Windows 手动连接”，再分别验证 Windows 与 Android 本地移除及全新重配。通过前不开始 P2-005 多设备核心。
