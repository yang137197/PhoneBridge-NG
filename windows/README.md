...
`PhoneBridge.Connection` 与 `PhoneBridge.Desktop` 已实现 WPF 首次配对、手机确认的只读/安全/完全读写挂载、安全模式写入/确认删除、按设备隔离的多会话与自动重连、托盘、自启动、本地脱敏诊断包和手动地址排障入口。每个设备会话独立持有挂载管理器、盘符、rclone/RC、取消、健康检查和重连状态；尚无持久盘符偏好。开发预览通过根目录 `./scripts/Start-WindowsPreview.ps1` 构建并打开；UI 验收必须运行打包候选并传入 `--ui-preview`，以使用按程序集版本隔离的数据根。v0.2.0 正式安装器包含自包含 .NET、固定 rclone，并在缺少 WinFsp 时运行已核验的官方 MSI；应用、安装器和托盘状态使用统一图标体系。日志与实际导出证据见 [P1-014](../docs/audit/P1-014-VALIDATION.md)，多设备证据见 [P2-005](../docs/audit/P2-005-VALIDATION.md)，安装边界见 [交付说明](../docs/LOCAL_DELIVERY.md)。
